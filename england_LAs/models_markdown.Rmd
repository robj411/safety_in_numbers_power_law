---
title: "Road traffic injuries: data and regression models"
...
inj_cols <- c('whw_bike_car','whw_ksi_bike_car','whw_fatal_bike_car')
...
And now let's look at the difference between all the areas and the urban areas (in pink, defined as the areas with >99% of the population living in an urban area (city, town or minor conurbation) in the 2011 census).


```{r urban}
par(mfrow=c(1,3))
for(i in 1:3) {
  plot(log(rural$Pedal.Cycles),log(rural[[inj_cols[i]]]),pch=16,col='navyblue',xlab='Distance cycled',ylab=paste0('Cyclist ',ylabs[i]))
  points(log(urban$Pedal.Cycles),log(urban[[inj_cols[i]]]),pch=16,col='hotpink',cex=0.8)
}
```

So is it possible that we get the safety-in-numbers results as a result of having all high-density (urban) areas having lower levels of cycling? I.e., all the high-cycling places are rural and sparse? Yes: there is a relationship between total road length and total cycling, with urban areas having the shortest total road length (excepting the Isles of Scilly) and not the most total cycling.

```{r distance vs road length}
plot(log(rural$Pedal.Cycles),log(rural$AB),pch=16,col='navyblue',xlab='Distance cycled',ylab='Total A+B road length')
points(log(urban$Pedal.Cycles),log(urban$AB),pch=16,col='hotpink',cex=0.8)
```

Now let's look at the rates (the numbers per distance travelled).
...
par(mfrow=c(1,3))
for(i in 1:3) {
  plot(log(rural$Pedal.Cycles),log(rural[[inj_cols[i]]]/rural$Pedal.Cycles),pch=15,col='navyblue',xlab='Distance cycled',ylab=paste0('Cyclist ',ylabs[i],' per distance cycled'))
  points(log(urban$Pedal.Cycles),log(urban[[inj_cols[i]]]/urban$Pedal.Cycles),pch=16,col='hotpink',cex=0.8)
}
```
...
### Total travel distance only as a predictor, for all, urban and rural, for all, ksi, and fatal.

```{r coefficient: one predictor, total travel}
coef_summary <- data.frame(all=c(),urban=c(),rural=c())
for(j in 1:3)
  for(i in 1:3){
    frml <- paste0(inj_cols[i],'~log(total_travel)')
...
### Population only as a predictor, for all, urban and rural, for all, ksi, and fatal.

```{r coefficient: one predictor, population}
coef_summary <- data.frame(all=c(),urban=c(),rural=c())
for(j in 1:3)
  for(i in 1:3){
    frml <- paste0(inj_cols[i],'~log(population)')
    mod <- glm(as.formula(frml),data=subset(datasets[[j]],Year==2011),family=poisson(),offset=log(Pedal.Cycles))
    coef_summary[i,j] <- mod$coefficients[2]+1
  }
rownames(coef_summary) <- ylabs
colnames(coef_summary) <- names(datasets)
...
```{r coefficient: one predictor, cycle}
coef_summary <- data.frame(all=c(),urban=c(),rural=c())
for(j in 1:3)
  for(i in 1:3){
...
### Motor distance only as a predictor, for all, urban and rural, for all, ksi, and fatal.

```{r coefficient: one predictor, car}
coef_summary <- data.frame(all=c(),urban=c(),rural=c())
for(j in 1:3)
  for(i in 1:3){
    frml <- paste0(inj_cols[i],'~log(Car)')
    mod <- glm(as.formula(frml),data=datasets[[j]],family=poisson(),offset=log(Pedal.Cycles))
    coef_summary[i,j] <- mod$coefficients[2]+1
  }
rownames(coef_summary) <- ylabs
...
coef_summary <- data.frame(pedal=c(),motor=c(),sum=c())
  for(i in 1:3){
    frml <- paste0(inj_cols[i],'~log(Pedal.Cycles)+log(Car)')
    mod <- glm(as.formula(frml),data=datasets[[1]],family=poisson(),offset=log(Pedal.Cycles)+log(Car))
...
coef_summary <- data.frame(pedal=c(),motor=c(),sum=c())
  for(i in 1:3){
    frml <- paste0(inj_cols[i],'~log(Pedal.Cycles)+log(Car)')
    mod <- glm(as.formula(frml),data=datasets[[2]],family=poisson(),offset=log(Pedal.Cycles)+log(Car))
    coef_summary[i,1:2] <- mod$coefficients[2:3]+1
    coef_summary[i,3] <- sum(coef_summary[i,1:2])
  }
rownames(coef_summary) <- ylabs
...
coef_summary <- data.frame(pedal=c(),motor=c(),sum=c())
  for(i in 1:3){
    frml <- paste0(inj_cols[i],'~log(Pedal.Cycles)+log(Car)')
    mod <- glm(as.formula(frml),data=datasets[[3]],family=poisson(),offset=log(Pedal.Cycles)+log(Car))
    coef_summary[i,1:2] <- mod$coefficients[2:3]+1
    coef_summary[i,3] <- sum(coef_summary[i,1:2])
  }
rownames(coef_summary) <- ylabs
...
```{r coefficients: all}
coef_summary <- data.frame(intercept=c(),pedal=c(),motor=c(),sum=c())
  for(i in 1:3){
    frml <- paste0(inj_cols[i],'~log(Pedal.Cycles)+log(Car)')
    mod <- glm(as.formula(frml),data=datasets[[1]],family=poisson(),offset=log(Pedal.Cycles)+log(Car)-log(AB))
    coef_summary[i,1] <- mod$coefficients[1]
    coef_summary[i,2:3] <- mod$coefficients[2:3]+1
    coef_summary[i,4] <- sum(coef_summary[i,2:3])
  }
rownames(coef_summary) <- ylabs
colnames(coef_summary) <- c('intercept','pedal','motor','sum')
coef_summary
```
...
coef_summary <- data.frame(intercept=c(),pedal=c(),motor=c(),sum=c())
  for(i in 1:3){
    frml <- paste0(inj_cols[i],'~log(Pedal.Cycles)+log(Car)')
    mod <- glm(as.formula(frml),data=datasets[[2]],family=poisson(),offset=log(Pedal.Cycles)+log(Car)-log(AB))
    coef_summary[i,1] <- mod$coefficients[1]
    coef_summary[i,2:3] <- mod$coefficients[2:3]+1
    coef_summary[i,4] <- sum(coef_summary[i,2:3])
  }
...
coef_summary <- data.frame(intercept=c(),pedal=c(),motor=c(),sum=c())
  for(i in 1:3){
    frml <- paste0(inj_cols[i],'~log(Pedal.Cycles)+log(Car)')
    mod <- glm(as.formula(frml),data=datasets[[3]],family=poisson(),offset=log(Pedal.Cycles)+log(Car)-log(AB))
    coef_summary[i,1] <- mod$coefficients[1]
    coef_summary[i,2:3] <- mod$coefficients[2:3]+1
    coef_summary[i,4] <- sum(coef_summary[i,2:3])
  }
...
par(mfrow=c(1,3))
for(i in 1:3){
  plot(log(rural$Pedal.Cycles/rural$AB),log(rural[[inj_cols[i]]]/rural$Pedal.Cycles),pch=16,col='navyblue',xlab='Cycle distance density',ylab=paste0('Cyclist ',ylabs[i],' per distance cycled'))
  points(log(urban$Pedal.Cycles/urban$AB),log(urban[[inj_cols[i]]]/urban$Pedal.Cycles),pch=16,col='hotpink',cex=0.8)
}
```
...
par(mfrow=c(1,3))
for(i in 1:3){
  plot(log(rural$Pedal.Cycles/rural$AB),log(rural[[inj_cols[i]]]),pch=16,col='navyblue',xlab='Cycle distance density',ylab=paste0('Cyclist ',ylabs[i]))
  points(log(urban$Pedal.Cycles/urban$AB),log(urban[[inj_cols[i]]]),pch=16,col='hotpink',cex=0.8)
}
```